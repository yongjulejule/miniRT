...
/*   Updated: 2021/12/16 17:19:42 by yongjule         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt_bonus.h"

static int	cy_shadow(double *ray, t_pt_info *pt_i, t_cy *cy, double r_size)
{
	int	shaded;

	shaded = side_cy_shadow(ray, pt_i, cy, r_size);
	if (!shaded)
		shaded = circle_cy_shadow(ray, pt_i, cy, r_size);
	return (shaded);
}

static int	cn_shadow(double *ray, t_pt_info *pt_i, t_cn *cn, double r_size)
{
	int	shaded;

	shaded = side_cn_shadow(ray, pt_i, cn, r_size);
	if (!shaded)
		shaded = circle_cn_shadow(ray, pt_i, cn, r_size);
	return (shaded);
}
